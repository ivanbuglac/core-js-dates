...
function getNextFridayThe13th(date) {
  ...
  for (let month = date.getMonth(); month < Infinity; month += 1) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  function dateNormalize(str) {
    return `${str.slice(6)}-${str.slice(3, 6)}${str.slice(0, 2)}`;
  }
  ...
  const end = new Date(dateNormalize(period.end));
  const start = new Date(dateNormalize(period.start));
  const arr = [period.start];
  let countWorkIterator = countWorkDays - 1;
  let countOffIterator = countOffDays;
  while (start < end) {
    if (countWorkIterator > 0) {
      start.setDate(start.getDate() + 1);
      arr.push(dateToStr(start));
      countWorkIterator -= 1;
      countOffIterator = countOffDays;
    } else if (countOffIterator > 1) {
      countOffIterator -= 1;
      start.setDate(start.getDate() + 1);
    } else {
      countWorkIterator = countWorkDays;
      countOffIterator = countOffDays;
      start.setDate(start.getDate() + 1);
    }
  }
